    ...
    def analyze_events(self, side):
        ...
        for asteroid in self.list_asteroids:
            if pygame.sprite.spritecollide(self.ball, asteroid, False):
                # self.game.list_asteroids.list_pos.remove(asteroid)
    ...
    def refresh(self,playing=True,loser=0):
        ...
        if playing:
            ...
            if loser != 2:
                ...
                font = pygame.font.Font(None, 74)
                if loser==-1:
                    text = font.render("YOU QUIT PLAYING", 1, GREEN)
                else:
    ...
    try:
        with Client((ip_address, 6000), authkey=b'secret password') as conn:
            ...
            while game.is_running():
                ...
                display.refresh()
                display.tick()
            display.refresh(playing=False,loser= game.get_loser())
            time.sleep(5)
    except:
        traceback.print_exc()
    finally:    
        # display.refresh(playing=False)
        # time.sleep(5)
        pygame.quit()
