...
class Game():
    def __init__(self, manager):
        ...
        self.list_asteroids = List_Asteroids(generate_aseroids(NUM_ASTEROIDS,manager))
    ...
    def ball_collide_player(self, player):
        self.lock.acquire()
        ball = self.ball[0]
    ...
    def ball_collide_asteroid(self, player):
        """
        No se porque se da el player como argumento si no lo es de por sí en la clase ball
        Parece ser que es debido a que así sabemos quien manda el mensaje

        ¿como añadir collide_steroid aqui?
        """
        self.lock.acquire()
        ball = self.ball[0]
        ball.collide_asteroid(player)
        self.ball[0] = ball
        self.lock.release()
    ...
    def get_info(self):
        info = {
            ...
            'loser': self.loser,
            'list_asteroids': list(self.list_asteroids.get_pos()),
        ...
        while game.is_running():
            ...
            while command != "next":
                ...
                if command == "left":
                    ...
                    game.ball_collide_player(side)
                elif command == "collide_asteroid":
                    game.ball_collide_asteroid(side)
                elif command == "quit":
                    game.stop()
            if side == 1:
                game.move_ball()
