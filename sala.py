...
class Game():
    def __init__(self, manager):
        ...
        # self.ball = manager.list( [ Ball([2,3]) ] )
    ...
    def destroy_asteroid(self, asteroid):
        self.list_asteroids[0].collide_ball(asteroid)
        self.asteroids_left.value-=1
            ...
            while command != "next":
                ...
                if command == "left":
                    ...
                    if side == 0:
                        ...
                        print(f"MANDANDO: {asteroid}")
                        game.ball_collide_asteroid()
                        game.destroy_asteroid(asteroid)
                        game.list_asteroids[0].list_pos.remove(asteroid)
                        print(len(game.list_asteroids[0].get_pos()))
                        print(game.list_asteroids[0].get_pos())
                        if game.asteroids_left.value==0:
                        # if len(game.list_asteroids[0].get_pos())==0:
                            game.loser.value = 2
                            game.stop()
                elif command == "quit":
                    game.stop()
